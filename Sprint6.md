...
6-6. [ ] Android UI sync fix: On Android, when device is paired to another one via cloud sync and 2nd device uses Clear Old Reminders, UI on the Android device still shows old reminders in Sent even after sync
6-7. [ ] Fix bug: Editing a reminder on Sent and Cancelled screens does not make the reminder active. Should make active if time is in the future, oherwise fail the Edit with error message saying "Reminder time should be in the future"
